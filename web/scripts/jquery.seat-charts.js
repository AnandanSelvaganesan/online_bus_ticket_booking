...
(function($) {
	...
	$.fn.seatCharts = function (setup) {
		...
		$.each(settings.map, function(row, characters) {
			 ...
			 * a[,LABEL]
			 *
			 * Allowed characters in IDs are: 0-9, a-z, A-Z, _
			...
			$.each(characters.match(/[a-z_]{1}(\[[0-9a-z_]{0,}(,[0-9a-z_ ]+)?\])?/gi), function (column, characterParams) { 
				...
				$row.append(character != '_' && character != 'e' ?
					...
					$('<div></div>').addClass(['seatCharts-cell', 'seatCharts-space'].concat(
								//let's merge custom user defined classes with standard JSC ones
								settings.classes, 
								typeof settings.seats[character] == "undefined" ? 
									[] : settings.seats[character].classes
								).join(' '))
				);
			});
